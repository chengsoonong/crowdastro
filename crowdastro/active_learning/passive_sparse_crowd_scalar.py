"""Yan et al. (2010) EM sparse crowd passive learning algorithm; scalar η_t.
...
from .passive_crowd import compute_majority_vote
...
def annotator_model(h, x, y, z):
    """Yan et al. (2010) Bernoulli annotator model, with scalar η_t.

    h: Annotator parameter η_t. (n_dim,) NumPy array
...
def Q(params, n_dim, n_annotators, n_samples, posteriors, posteriors_0, x, y):
    ...
    expectation = numpy.ma.sum(
            numpy.ma.dot(posteriors,
                (numpy.ma.log(annotator_model(h, x, y, 1) + EPS) +
                 numpy.ma.log(logistic_regression(a, b, x) + EPS)).T) +
            numpy.ma.dot(posteriors_0, (
                    numpy.ma.log(annotator_model(h, x, y, 0) + EPS) +
    ...
    for t in range(n_annotators):
        dQ_dh[t] += sum(-posteriors[i] * (h[t] + abs(y[t, i] - 1) - 1)
                                / ((1 - h[t]) * h[t]) +
                        -posteriors_0[i] * (h[t] + abs(y[t, i] - 0) - 1)
                                / ((1 - h[t]) * h[t])
...
def em_step(n_samples, n_annotators, n_dim, a, b, h, x, y):
    ...
    posteriors = lr.copy()
    posteriors *= numpy.ma.prod(annotator_model(h, x, y, 1), axis=0)

    # Repeat for p(z_i = 0 | x_i, y_i).
    posteriors_0 = 1 - lr
    posteriors_0 *= numpy.ma.prod(annotator_model(h, x, y, 0), axis=0)
...
def train(x, y, epsilon=1e-5, lr_init=False, skip_zeros=False):
    ...
    h = numpy.random.normal(size=(n_annotators,))
