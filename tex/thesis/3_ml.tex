...
\section{Crowdsourced Labelling}
    ...
    \subsection{Modelling Labeller Reliability}
        ...
        \subsubsection{Raykar et al. (2010) Method}
            ...
            \end{align*}
            and use the Newton-Raphson method to find $\vec w$ with gradient
            \begin{equation*}
                \vec g(w) = \sum_{i = 1}^N \left(\mu_i - \sigma(\vec w^T \vec x_i\right) \vec x_i
            \end{equation*}
            and Hessian
            \begin{equation*}
                H(w) = -\sum_{i = 1}^N \sigma(\vec w^T \vec x_i) (1 - \sigma(\vec w^T \vec x_i)) \vec x_i \vec x_i^T.
            \end{equation*}
