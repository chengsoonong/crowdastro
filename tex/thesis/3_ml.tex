...
\label{sec:image-features}
    ...
    \label{sec:naive-approach-image-features}
        ...
        features from images, but these features fail to accurately capture the
        image. The main reason for this is that the pixels are assumed
        ...
        further correlations, and so on. This is shown in Figure
        \ref{fig:shuffled-face}. If we are only looking at certain kinds of
        images, then the pixel location may also matter --- for example, we
        might have as inputs photographs of landscapes, where the sky tends to
        be blue and tends to be at the top of the image.
