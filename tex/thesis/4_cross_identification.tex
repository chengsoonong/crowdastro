...
\label{sec:experiment-design}
  ...
  \citeauthor{norris06} labels and the \citeauthor{fan15} labels agree. To
...
\label{sec:features}
  ...
  \label{sec:ir-features}
    ...
    --- the AllWISE catalogue (Section \ref{sec:wise}) and the SWIRE catalogue
    ...
    Future work may investigate the effect of features extracted from infrared
    images on classification performance, but this is beyond the scope of this
    thesis.
    ...
    centre of the closest radio object in ATLAS.
  ...
  \label{sec:radio-features}
    ...
    \label{sec:feature-extraction-model}
      ...
      training set. The entire network was then trained for 10~000 epochs to
      match the consensus Radio Galaxy Zoo labels of the galaxies nearest the
      reserved training set.

      A better approach would be to use a convolutional autoencoder, which would
      allow training on all the radio data and on data with no labels at all,
      but this was computationally infeasible for this project.
...
\label{sec:binary-classifiers}
  ...
  \begin{figure}
    ...
    \caption{Comparison of logistic regression and random forests on the galaxy
      ...
      label sets, and tested against the \citet{norris06} label set. Spread
  ...
  % objects. We then added all WISE objects within $1'$ of a selected ATLAS
  % object to the test set. This was repeated for each test set. The motivation
  % for first selecting ATLAS objects rather than drawing WISE objects directly
  % is that WISE objects have overlapping features, since radio features are
  % taken from a patch of sky centred on each object, and WISE objects tend to
  % be close together. Objects from the test sets were then removed at random
  % until all test sets were the same size.

  We found that logistic regression greatly outperformed random forests. We
  therefore decided to use logistic regression for later experiments.
...
\label{sec:rgz-crowd-labels}
  ...
  measures: the 15 labellers with the most labels, the 15 labellers with the
  highest balanced accuracy assessed against the \citeauthor{norris06} labels,
  and the 15 labellers with the highest balanced accuracy assessed against the
...
\label{sec:rgz-results}
  ...
  \label{sec:comparison-predictors}
    ...
    \begin{figure}
      ...
      \caption{Performance of logistic regression and the \citeauthor{raykar10}
        ...
        trained on $Y$. RGZ-MV is the training set of majority vote labels from
        the Radio Galaxy Zoo. RGZ-Raw-MV is the majority vote of all crowd
        labels and is included for comparison. Spread along the vertical axis
        represents results across multiple test sets; the bar represents the
        mean.}
  ...
  \label{sec:raykar-estimates-rgz}
    ...
    values of $\alpha$ were weakly correlated, with $r = 0.11$; the true and
    predicted values of $\beta$ were not, with $r = -0.01$. According to the
    Spearman correlation coefficient, the true and predicted values of $\alpha$
    were weakly correlated, with $r = 0.32$; the true and predicted values of
    $\beta$ were also correlated, with $r = 0.21$.

    We note that the Pearson correlation coefficient assumes that the values
    being tested are normally distributed, which the $\alpha$ and $\beta$ values
    are not.
