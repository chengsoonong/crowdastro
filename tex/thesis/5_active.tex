...
\label{sec:active-learning-on-crowds}
    ...
    problem of active learning from crowds. We remind the reader that this model consists of a label model $p(z \mid x)$ and a data-dependent labeller model $p(y_t | x, z)$, where $x$ is an instance, $y_t$ is the label assigned by labeller $t$, and $z$ is the groundtruth label. To extend this model into active learning, \citeauthor{yan11} introduce a query strategy where not only an instance is requested, but a specific labeller. The labeller and instance to query are chosen as solutions to the following optimisation problem:
    \begin{align*}
        \text{minimise } & p(y_t \mid x_i, z_i)\\
        \text{s.t. } & 
    \end{align*}
    \todo{finish this}
